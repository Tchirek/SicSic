...
function openCard(title: string, build: (body: HTMLElement) => void): void {
  closeOverlay();
...
export function confirmVisit(url: string, displayName: string): void {
  openCard('前往个人网站', (body) => {
    const hint = document.createElement('p');
    hint.className = 'auth-hint';
    hint.textContent = `即将离开当前页面，前往 ${displayName} 的个人网站：`;
...
export function showProfileCard(profile: PublicProfile, authOrigin: string): void {
  openCard('个人档案', (body) => {
    ...
    if (profile.website) {
      ...
      link.addEventListener('click', (event) => {
        ...
        confirmVisit(profile.website!, profile.displayName);
      });
      addRow('网站', link);
    }
    if (profile.email) {
    ...
    if (!profile.bio && rows.childElementCount === 0) {
      ...
      empty.textContent = 'TA 还没有公开更多信息。';
      body.append(empty);
    }
  });
}
